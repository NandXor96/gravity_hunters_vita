...
static float overlay_campaign_details_render_multiline(Renderer *r, float x, float y, float line_spacing, const char *text, const float max_line_width)
{
    ...
    while (*ptr)
    {
        ...
        line[line_end] = '\0';
        renderer_draw_text(r, line, x, current_y, (TextStyle){0});
        free(line);
...
void overlay_campaign_details_render(Scene *s, struct Renderer *r)
{
    ...
    renderer_draw_filled_rect(r, backdrop, (SDL_Color){0, 0, 0, 160});
    ...
    renderer_draw_filled_rect(r, panel, (SDL_Color){24, 24, 24, 235});
    ...
    renderer_draw_text_centered(r, display_name, panel_x + panel_w * 0.5f, title_y, (TextStyle){0});
    ...
    renderer_draw_rect_outline(r, button_rect, (SDL_Color){255, 200, 80, 255}, 2);
