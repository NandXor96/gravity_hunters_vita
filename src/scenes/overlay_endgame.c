...
/* Stored stats populated via overlay_endgame_set_stats() */
...
void overlay_endgame_render(Scene *s, struct Renderer *r)
{
    ...
    y += 36;
    snprintf(buf, sizeof(buf), "Kills: %d", g_kills);
    renderer_draw_text_centered(r, buf, (float)cx, (float)y, (TextStyle){0});
    y += 28;
    snprintf(buf, sizeof(buf), "Deaths: %d", g_deaths);
    renderer_draw_text_centered(r, buf, (float)cx, (float)y, (TextStyle){0});
    y += 28;
    snprintf(buf, sizeof(buf), "Points: %d", g_points);
    renderer_draw_text_centered(r, buf, (float)cx, (float)y, (TextStyle){0});
