...
void scene_quick_play_menu_render(Scene *s, struct Renderer *r) {
    ...
    if (i == 5 && selected) {
        ...
        if (i >= 0 && i <= 4) {
            ...
            if (i == 1) {
                label = "Planet Size"; val = size_labels[st->planet_size];
            }
            if (i == 2) {
                label = "Difficulty"; val = difficulty_labels[st->difficulty];
            }
            if (i == 3) {
                label = "Time Limit"; val = time_labels[st->time_limit];
            }
            if (i == 4) {
                label = "Worlds"; val = worlds_labels[st->worlds];
            }

            /* draw label left inside main box */
            renderer_draw_text(r, label, label_x, text_y, (TextStyle) {
                0
            });
            ...
            renderer_draw_filled_rect(r, inner_box, (SDL_Color) {
                24,24,24,255
            });
            /* inner outline only; highlight when selected */
            if (selected) {
                renderer_draw_rect_outline(r, inner_box, (SDL_Color){255,200,80,255
            }, 2);
            }
            else {
                renderer_draw_rect_outline(r, inner_box, (SDL_Color) {
                    80,80,80,255
                }, 1);
            }
            ...
            renderer_draw_text(r, "<", chev_left_x, text_y, (TextStyle) {
                0
            });
            renderer_draw_text(r, val, val_x, text_y, (TextStyle) {
                0
            });
            renderer_draw_text(r, ">", chev_right_x, text_y, (TextStyle) {
                0
            });
        }
        if (i == 5) {
