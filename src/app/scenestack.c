...
void scenestack_init(SceneStack *st) { memset(st, 0, sizeof(*st)); }
...
void scenestack_update(SceneStack *st, float dt)
{
    if (st->base)
    {
        ...
        {
            // Update overlays top->down, base only if no overlay blocks
            bool blocked = false;
            for (int i = st->overlay_count - 1; i >= 0; i--)
            {
                Scene *s = st->overlays[i];
