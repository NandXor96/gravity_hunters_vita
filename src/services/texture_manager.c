...
SDL_Rect texman_sheet_src(TextureManager *m, TextureID id, int index) { return sheet_src(m, id, index); }
SDL_Texture *texman_projectiles_texture(TextureManager *m) { return m ? m->base[TEX_PROJECTILES_SHEET].tex : NULL; }
...
int texman_projectile_variant_count(TextureManager *m) { return m ? m->projectile_variant_count : 0; }
...
static inline Uint32 px_make(Uint8 r, Uint8 g, Uint8 b, Uint8 a) { return ((Uint32)a << 24) | ((Uint32)r << 16) | ((Uint32)g << 8) | b; }
...
SDL_Texture *texman_explosions_texture(TextureManager *m) { return m ? m->explosions_sheet : NULL; }
int texman_explosion_type_count(TextureManager *m) { return m ? m->explosion_types : 0; }
int texman_explosion_frames(TextureManager *m) { return m ? m->explosion_frames : 0; }
...
static inline Uint32 make_color(Uint8 r8, Uint8 g8, Uint8 b8, Uint8 a8) { return ((Uint32)a8 << 24) | ((Uint32)r8 << 16) | ((Uint32)g8 << 8) | b8; }
static SDL_Texture *gen_nebula(SDL_Renderer *r, uint32_t seed)
{
    const int w = 960, h = 544;
