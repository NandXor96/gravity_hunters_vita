...
void ui_draw_ok_back_prompts(struct Renderer *r, struct Services *svc, bool include_back) {
    ...
    SDL_Point ok_size = renderer_measure_text(r, ok_label, (TextStyle){0});
    SDL_Point back_size = renderer_measure_text(r, back_label, (TextStyle){0});

    ok_size.x += 5;
    back_size.x += 5;

    float ok_icon_width = ui_prompt_icon_width(tex_ok, icon_height);
    float back_icon_width = include_back ? ui_prompt_icon_width(tex_back, icon_height) : 0.f;

    float ok_block_width = ok_icon_width;
    if (ok_size.x > 0) {
    ...
    renderer_draw_text(r, ok_label, current_x, ok_text_y, (TextStyle) {
        0
    });

    if (include_back) {

        current_x = start_x + ok_block_width + prompt_spacing;
        ...
        renderer_draw_text(r, back_label, current_x, back_text_y, (TextStyle) {
            0
        });
    }
}
