// -----------------------------------------------------------------------------
// collision.c
// Zentraler Kollisions-Durchlauf für alle aktiven Entities (Player, Enemies,
// Planets, Projectiles). Dieses Modul:
//   1. Sammelt relevante Entities aus der World.
//   2. Bereitet (transformiert) optionale Polygon-Collider von lokal -> Welt.
//   3. Führt eine sehr leichte Broadphase (Bounding-Circle Overlap) aus;
//      jeder Entity besitzt einen Kreisradius (collider.radius) der schnell
//      getestet werden kann.
//   4. Führt Narrowphase Tests durch:
//        - Kreis vs Kreis
//        - Polygon vs Polygon (SAT – aktuell nur Overlap-Bool, ohne Penetration)
//        - Kreis vs Polygon (Approx: Punkt-in-Poly + kürzeste Edge-Dist <= r)
//   5. Ruft bei erkannten Treffern Dispatch auf, welches differenziert:
//        - Projektil trifft eine andere Entity -> nur Ziel bekommt on_hit
//          (Projectile selbst wird deaktiviert, KEIN on_collide Aufruf)
//        - Alle anderen Paarungen -> beidseitiger on_collide Callback
//   6. Setzt am Ende poly_world_dirty zurück, damit sich Polygone bei Rotation
//      oder Positionsänderung im nächsten Frame erneut korrekt transformieren.
// WICHTIG / DESIGN ENTSCHEIDUNGEN:
// - Es gibt KEINE globale physische Separierung oder Bounce mehr im System.
//   Dies wurde bewusst entfernt: Entities entscheiden selbst innerhalb ihrer
//   on_collide (oder in Zukunft erweiterten on_collide(info)) Routinen über
//   Reaktionen (Schaden, Bounce, Energieverlust, etc.).
// - Friendly-Fire der Projektile wird schon im Filter ausgeschlossen.
// - Projektile erhalten niemals einen on_collide Call, um doppelte Logik und
//   ungewollte physische Reaktionen zu vermeiden; nur das Ziel verarbeitet den
//   Treffer via on_hit.
// - Penetration & Normal werden derzeit nicht berechnet/weitergereicht; SAT
//   dient nur als Overlap-Erkennung. Eine spätere Erweiterung (CollisionResult)
//   könnte diese Daten wieder einführen, sobald Bounce/Response benötigt wird.
// - Broadphase nutzt nur schnelle Kreis-Kollision (Bounding Circle), kein
//   AABB mehr – dadurch weniger Code & Rotation egal.
// - Polygone werden im EntityCollider in zwei Arrays verwaltet:
//     poly_local[]  -> unveränderte lokale Punkte (Pixelkoordinaten)
//     poly_world[]        -> transformierte Weltpunkte (gültig wenn poly_world_dirty=0)
// PERFORMANCE HINWEISE / ZUKUNFT:
// - Circle-Poly aktuell approximativ. Exakter SAT (Circle Projection) kann
//   ergänzt werden, falls Präzision erforderlich.
// -----------------------------------------------------------------------------
#include "collision.h"
#include "world.h"
#include "player.h"
