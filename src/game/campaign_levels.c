...
static void derive_display_name(const char *filename, char *buffer, size_t buffer_size)
{
...
{
    fprintf(stderr, "[campaign_levels] Skipping level '%s' (name too long, max %d chars)\n",
...
int campaign_levels_scan(CampaignLevelList *list, const char *levels_root)
{
    ...
    if (!dir)
    {
        fprintf(stderr, "[campaign_levels] Failed to open '%s': %s\n", root, strerror(errno));
        return -1;
    }
...
void campaign_level_info_init(CampaignLevelInfo *info, const char *filename)
{
    ...
    derive_display_name(info->filename[0] ? info->filename : filename, info->display_name, sizeof(info->display_name));
    info->unlocked = false;
    info->stars_earned = 0;
}
