...
static void planet_destroy_entity(Entity *e) { planet_destroy((Planet *)e); }
...
void planet_destroy(Planet *p) { free(p); }
Circle planet_collider(const Planet *p) { return (Circle){{p->e.pos.x, p->e.pos.y}, p->radius}; }
